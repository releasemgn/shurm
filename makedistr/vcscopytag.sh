...
function f_local_vcs_copytag_svn() {
	...
	# check source status
	local CHECK_NOT_EXISTS=`svn info $P_SVNAUTH $P_SVNPATH/$P_VCS_PATH/$MODULE/tags/$TAG 2>&1 | grep "Not a valid"`

	if [ "$CHECK_NOT_EXISTS" != "" ]; then
		echo $P_SVNPATH/$P_VCS_PATH/$MODULE/tags/$TAG: svn path does not exist. Exiting
		exit 1
	fi

	# check destination status
	CHECK_NOT_EXISTS=`svn info $P_SVNAUTH $P_SVNPATH/$P_VCS_PATH/$MODULE/$TARGET 2>&1 | grep "Not a valid"`

	if [ "$CHECK_NOT_EXISTS" = "" ]; then
		echo drop new tag - already exists...
		svn delete $P_SVNAUTH $P_SVNPATH/$P_VCS_PATH/$MODULE/$TARGET -m "$C_CONFIG_ADM_TRACKER-0000: drop tag before svncopy"
	fi

	svn copy $P_SVNAUTH $P_SVNPATH/$P_VCS_PATH/$MODULE/tags/$TAG $P_SVNPATH/$P_VCS_PATH/$MODULE/$TARGET -m "$C_CONFIG_ADM_TRACKER-0000: copy tag"
}
...
echo vcscopytag.sh: finished MODULE=$MODULE, MODULEPATH=$MODULEPATH, TAG=$TAG, TARGET=$TARGET
