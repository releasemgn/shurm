...
function f_local_download_one() {
	...
	if [ "$P_TYPE" = "pgustatic" ]; then
		local WAR_FILENAME=$P_ARTEFACTID-$DOWNLOAD_VERSION.war
		local STATIC_FILENAME=$P_ARTEFACTID-$DOWNLOAD_VERSION-webstatic.tar.gz

		f_downloadnexus_and_copydistr $P_PROJECT $C_CONFIG_NEXUS_REPO $P_GROUPID $P_ARTEFACTID $DOWNLOAD_VERSION "war"
		f_downloadnexus $P_PROJECT $C_CONFIG_NEXUS_REPO $P_GROUPID $P_ARTEFACTID $DOWNLOAD_VERSION "tar.gz" "webstatic"
		f_repackage_staticdistr $P_PROJECT $DOWNLOAD_VERSION $WAR_FILENAME $STATIC_FILENAME

	elif [ "$P_TYPE" = "static" ]; then
		f_downloadnexus_and_copydistr $P_PROJECT $C_CONFIG_NEXUS_REPO $P_GROUPID $P_ARTEFACTID $DOWNLOAD_VERSION "war"
		f_downloadnexus_and_copydistr $P_PROJECT $C_CONFIG_NEXUS_REPO $P_GROUPID $P_ARTEFACTID $DOWNLOAD_VERSION "tar.gz" "$F_CLASSIFIER"

	else
		local F_EXTPACKAGING=${C_SOURCE_ITEMEXTENSION##*.}
		f_downloadnexus_and_copydistr $P_PROJECT $C_CONFIG_NEXUS_REPO $P_GROUPID $P_ARTEFACTID $DOWNLOAD_VERSION "$F_PACKAGING" "$F_CLASSIFIER" "$C_SOURCE_ITEMFOLDER"
	fi
}
