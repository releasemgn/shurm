...
function f_execute_alltags() {
	...
	# execute
	echo MODULESET=$MODULESET, MODULE=$MODULE, TAG=$TAG, VERSION=$MODULEVERSION, MODULEOPTIONS=$MODULEOPTIONS
	mkdir -p $BUILDOUTDIR

	# checkout sources
	local F_NEXUS_PATH=$C_CONFIG_NEXUS_BASE/content/repositories/$C_CONFIG_NEXUS_REPO
	./patch.sh $MODULESET $MODULE $MODULEPATH $TAG $MODULEVERSION $F_NEXUS_PATH $MODULEOPTIONS $BUILDOUTDIR
	if [ $? -ne 0 ]; then
		BUILDSTATUS=FAILED
	else
