...
function f_local_executelocation_binary() {
	...
	f_redist_rollout_generic $P_SERVER $P_HOSTLOGIN $P_NODE $P_ROOTDIR $P_SRCVERSIONDIR $P_LOCATION $F_LOC_DEPLOYTYPE "$P_HOTUPLOADPATH" "$P_LINKFROMPATH"
	local F_STATUS=$?
	if [ "$F_STATUS" = "0" ]; then
		S_DEPLOY_STATUS=yes
	fi
...
function f_local_executelocation_config() {
	...
	echo execute config locationdir=$P_LOCATION...
	f_redist_rollout_config $P_SERVER $P_HOSTLOGIN $P_NODE $P_ROOTDIR $P_SRCVERSIONDIR $P_LOCATION

	# save in svn if keep alive configuration
	if [ "$GETOPT_KEEPALIVE" = "yes" ]; then
		# get components affected
		f_redist_getitems $P_SERVER $P_HOSTLOGIN $P_SRCVERSIONDIR $P_LOCATION "config"

		if [ "$C_REDIST_DIRITEMS_CONFIG" != "" ]; then
...
function f_local_executenode_binary() {
	...
	if [ "$F_LOCATIONS" = "" ]; then
		echo "$P_HOSTLOGIN: no binaries to roll out"
		return 1
	fi
...
function f_local_executenode_config() {
	...
	if [ "$F_LOCATIONS" = "" ]; then
		echo "$P_HOSTLOGIN: no configuration locations to roll out"
	else
		# execute by location
		local locationdir
		for locationdir in $F_LOCATIONS; do
...
function f_local_executenode() {
	...
	echo "$P_HOSTLOGIN: start rollout, see log $F_SERVER_DEPLOYLOG ..."
...
function f_local_wait_rollout() {
	local F_PROC
	local F_FILE

	echo "`date`: wait for rollout..."
	local F_STATUS
	...
	for F_PROCONE in $F_PROCS; do
		...
		if [ "$F_STATUS" != "0" ]; then
			...
			echo "f_local_wait_rollout: error rolling out, see log file ($F_FILE). Exiting"
			exit 1
		fi
	done

	echo "`date`: wait for rollout successfully finished"
}
...
function f_local_executedc() {
	...
	# wait rollout using total.txt
	f_local_wait_rollout
}
...
function f_local_execute_all() {
	...
	echo rollout release=$P_SRCVERSIONDIR SRVNAME_LIST=$SRVNAME_LIST...

	# rollout all std binaries (except for windows-based)
	...
	if [ "$C_REDIST_EXECUTE_ECHO_ONLY" = "true" ]; then
		echo "rollout.sh: roll out distribution to runtime (show only)..."
	else
		echo "rollout.sh: roll out distribution to runtime (execute)..."
	fi
...
echo rollout.sh: SUCCESSFULLY DONE.
