...
function f_local_executestaticnode() {
	...
	f_redist_transfer_staticset $P_STATICSERVER $P_LOCAL_HOSTLOGIN $P_ROOTDIR "$P_DISTITEMS" $P_SRCVERSIONDIR $P_LOCATION $S_REDIST_SRCDIR $S_REDIST_DISTR_REMOTEHOST
}
...
function f_local_executelocation() {
	...
	if [ "$F_REDIST_DIST_ITEMS" != "" ]; then
		...
		f_redist_transfer_fileset $P_SERVER $F_LOC_DEPLOYTYPE $P_HOSTLOGIN $P_ROOTDIR "$F_REDIST_DIST_ITEMS" $P_SRCVERSIONDIR $F_LOCATIONFINAL "$P_LINKFROM_DIR" $S_REDIST_SRCDIR $S_REDIST_DISTR_REMOTEHOST
	fi
