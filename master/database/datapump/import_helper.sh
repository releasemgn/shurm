...
function f_execute_all_importdatafull() {
	...
	for schema in $P_SCHEMALIST; do
		...
		if [ ! -f "$S_LOAD_ORACLEDIR/$F_DUMP" ]; then
			...
			f_impdp $S_CONNECTION "DIRECTORY=$C_ENV_CONFIG_DATAPUMP_DIR SCHEMAS=$F_SCHEMA_UPPER INCLUDE=TABLE_DATA TABLE_EXISTS_ACTION=TRUNCATE DUMPFILE=$F_DUMP LOGFILE=$schema.log $F_REMAP $F_TRANSFORM" ignoreerrors
		fi
	done
