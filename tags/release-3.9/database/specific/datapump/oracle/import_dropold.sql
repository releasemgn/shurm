...
FROM 
	( SELECT DISTINCT TABLESPACE_NAME AS TABLESPACE_NAME FROM dba_extents a WHERE owner IN ( @SCHEMA_LIST@ ) union 
		SELECT TABLESPACE_NAME FROM DBA_TABLESPACES WHERE REPLACE( TABLESPACE_NAME , '_DB' , '' ) in ( @SCHEMA_LIST@ ) ) a ,
	DBA_DATA_FILES c 
WHERE
