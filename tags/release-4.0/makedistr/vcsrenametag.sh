...
function f_local_vcs_renametag_svn() {
	...
	if [ "$CHECK_NOT_EXISTS" = "" ]; then
		...
		svn delete $P_SVNAUTH $P_SVNPATH/$P_VCS_PATH/$REPOSITORY/tags/$TAG2 -m "$C_CONFIG_ADM_TRACKER-0000: drop tag before svnrename"
	fi

	svn rename $P_SVNAUTH $P_SVNPATH/$P_VCS_PATH/$REPOSITORY/tags/$TAG1 $P_SVNPATH/$P_VCS_PATH/$REPOSITORY/tags/$TAG2 -m "$C_CONFIG_ADM_TRACKER-0000: rename tag"
}
...
echo vcsrenametag.sh: finished MODULE=$MODULE, REPOSITORY=$REPOSITORY, MODULEPATH=$MODULEPATH, TAG1=$TAG1, TAG2=$TAG2
