...
# execute
function f_local_vcs_copybranch_svn() {
	local P_VCS_PATH=$1
	local P_SVNPATH=$2
	local P_SVNAUTH=$3
	...
	svn copy $P_SVNAUTH $P_SVNPATH/$P_VCS_PATH/$REPOSITORY/$BRANCH1X $P_SVNPATH/$P_VCS_PATH/$REPOSITORY/$BRANCH2X -m "$C_CONFIG_ADM_TRACKER-0000: copy branch"
}

function f_local_vcs_copybranch_git() {
	local P_VCS_PATH=$1
...
function f_local_vcs_copybranch() {
	local MODULE_PATH_TYPE=${MODULEPATH%%:*}
	local MODULE_PATH_DATA=${MODULEPATH##*:}

	if [ "$MODULE_PATH_TYPE" = "svn" ]; then
		f_local_vcs_copybranch_svn $MODULE_PATH_DATA $C_CONFIG_SVNOLD_PATH "$C_CONFIG_SVNOLD_AUTH"

	elif [ "$MODULE_PATH_TYPE" = "svnnew" ]; then
		f_local_vcs_copybranch_svn $MODULE_PATH_DATA $C_CONFIG_SVNNEW_PATH "$C_CONFIG_SVNNEW_AUTH"

	elif [ "$MODULE_PATH_TYPE" = "git" ]; then
		f_local_vcs_copybranch_git $MODULE_PATH_DATA
...
f_local_vcs_copybranch

echo vcscopybranch.sh: finished MODULE=$MODULE, REPOSITORY=$REPOSITORY, MODULEPATH=$MODULEPATH, BRANCH1=$BRANCH1, BRANCH2=$BRANCH2
