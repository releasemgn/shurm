...
function f_local_execute_server() {
	...
	if [ "$F_SERVER_TYPE" = "service" ]; then
		f_process_waitall_service_started $DC $P_SRVNAME $C_ENV_SERVER_SERVICENAME "$C_ENV_SERVER_HOSTLOGIN_LIST" "$NODE_LIST"
		...
		f_process_waitall_generic_started $DC $P_SRVNAME "$C_ENV_SERVER_HOSTLOGIN_LIST" "$C_ENV_SERVER_ROOTPATH" "$C_ENV_SERVER_BINPATH" "$NODE_LIST"
