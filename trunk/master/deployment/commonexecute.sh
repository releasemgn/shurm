...
function f_common_execute_runcmd() {
	...
	local P_FUNCTION=$3
	local P_SERVERSET=$4
	local P_SERVER_LIST="$5"
	local P_NODE_LIST="$6"
	local P_GROUP=$7
	local P_SERVER=$8
	local P_NODE=$9
	local P_HOSTLOGIN=${10}
...
# build operations
		RUNCMD)
			f_common_execute_runcmd $P_SUB $P_DC $P_FUNCTION $P_SERVERSET "$P_SERVER_LIST" "$P_NODE_LIST" $P_GROUP $P_SERVER $P_NODE $P_HOSTLOGIN
			;;
	esac
}
...
function f_common_execute_node() {
	...
	f_common_execute_function "executenode" $P_DC $P_FUNCTION $P_SERVERSET "$P_SERVER_LIST" "$P_NODE_LIST" $P_GROUP $P_SERVER $P_NODE $P_HOSTLOGIN
}
...
function f_common_execute_server() {
	...
	f_execute_function "startserver" $P_DC $P_FUNCTION $P_SERVERSET "$P_SERVER_LIST" "$P_NODE_LIST" $P_GROUP $P_SERVER
	...
	for hostlogin in $C_ENV_SERVER_HOSTLOGIN_LIST; do
		if [ "$P_NODE_LIST" = "" ] || [[ " $P_NODE_LIST " =~ " $NODE " ]]; then
			f_common_execute_node $P_DC $P_FUNCTION $P_SERVERSET "$P_SERVER_LIST" "$P_NODE_LIST" $P_GROUP $P_SERVER $NODE $hostlogin
		fi
		NODE=$(expr $NODE + 1)
	done

	f_execute_function "finishserver" $P_DC $P_FUNCTION $P_SERVERSET "$P_SERVER_LIST" "$P_NODE_LIST" $P_GROUP $P_SERVER
}
...
function f_common_execute_group() {
	...
	S_EXECUTE_SERVERS=
	f_execute_function "getservers" $P_DC $P_FUNCTION $P_SERVERSET "$P_SERVER_LIST" "$P_NODE_LIST" $P_GROUP

	# iterate servers	
	local server
	for server in $S_EXECUTE_SERVERS; do
		# execute server
		f_common_execute_server $P_DC $P_FUNCTION $P_SERVERSET "$F_SERVER_LIST" "$P_NODE_LIST" $P_GROUP $server
	done

	f_execute_function "finishgroup" $P_DC $P_FUNCTION $P_SERVERSET "$P_SERVER_LIST" "$P_NODE_LIST" $P_GROUP
}
...
function f_common_execute_set() {
	...
	# split into groups
	f_execute_function "getgroups" $P_DC $P_FUNCTION $P_SERVERSET "$F_SERVER_LIST" "$S_EXECUTE_PARAM_NODES"

	# iterate groups
	local group
	for group in $S_EXECUTE_GROUPS; do
		# execute group
		f_common_execute_group $P_DC $P_FUNCTION $P_SERVERSET "$F_SERVER_LIST" "$S_EXECUTE_PARAM_NODES" $group 
	done

	f_execute_function "finishdc" $P_DC $P_FUNCTION $P_SERVERSET "$P_SERVER_LIST" "$S_EXECUTE_PARAM_NODES"
}
