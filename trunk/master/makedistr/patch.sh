...
if [ "$P_REPOSITORY" = "" ]; then
	echo patch.sh: P_REPOSITORY not set
	exit 1
fi
...
function f_execute_all() {
	echo patch.sh: VERSION_MODE=$VERSION_MODE, P_MODULESET=$P_MODULESET, P_MODULENAME=$P_MODULENAME, P_REPOSITORY=$P_REPOSITORY, P_MODULEPATH=$P_MODULEPATH, P_TAG=$P_TAG, P_VERSION=$P_VERSION, P_NEXUS_PATH=$P_NEXUS_PATH, P_MODULEOPTIONS=$P_MODULEOPTIONS, P_BUILDOUTDIR=$P_BUILDOUTDIR > $P_BUILDOUTDIR/$P_MODULENAME-build.log

	local PATCHPATH=$C_CONFIG_BUILDPATH/$VERSION_MODE/$P_MODULENAME

	echo rm -rf $PATCHPATH... >> $P_BUILDOUTDIR/$P_MODULENAME-build.log 2>&1
	rm -rf $PATCHPATH 

	# checkout sources
	./patch-checkout.sh $PATCHPATH $P_MODULESET $P_MODULENAME $P_REPOSITORY $P_MODULEPATH $P_TAG >> $P_BUILDOUTDIR/$P_MODULENAME-build.log 2>&1
	if [ $? -ne 0 ]; then
