...
function f_env_getxmlserverinfo() {
	...
	f_env_getxmllineattr server "stoptime"
	C_ENV_SERVER_STOPTIME=$C_ENV_XMLVALUE
	...
	if [ "$C_ENV_SERVER_TYPE" = "service" ]; then
		...
		f_env_getxmllineattr server "dbmstype"
		C_ENV_SERVER_DBMSTYPE=$C_ENV_XMLVALUE
		if [ "$C_ENV_SERVER_DBMSTYPE" = "" ]; then
			C_ENV_SERVER_DBMSTYPE="oracle"
		fi
